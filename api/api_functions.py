...
def get_schedules():
  ...
  return df.reset_index().drop(columns={'index'}).set_index([df.home_team, df.boxscore_index], df.away_team).sort_index()
...
def get_players():
  ...
  return player_df.reset_index().rename(columns={'level_0': 'season'}).set_index([player_df.name, player_df.season]).sort_index()
...
def get_games():
  ...
  return seasons.reset_index().drop(columns={'index'}).set_index([seasons.home_team, seasons.date])
